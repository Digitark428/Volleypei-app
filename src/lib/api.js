...
export async function createTournoi(form, afficheFile, organisateurEmail) {
  ...
  if (!supabaseConfigured)
    return { id: Date.now(), ...form, affiche_url: null, affiche: form.affiche || null, joueurs: parseInt(form.joueurs)||0, created_at: new Date().toISOString() };
...
export async function getVisitesStats(nbJours = 30) {
  ...
  const { data, error } = await supabase
    .from('visites').select('jour, nb')
    .gte('jour', depuisStr).order('jour', { ascending: true });
